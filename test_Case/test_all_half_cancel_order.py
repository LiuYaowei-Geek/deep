    ...
    def test_1_shuj(self):
        ...
        for i in token_test['token']:
            ...
            if type_c[0] ==2:  # 判断是否全仓还是逐仓
                ...
                contract_type = {"contract_type": '2'}  # 是逐仓换成全仓
            ...
            xiadan=float(totoal)-float(totoal_balancel_1)
    ...
    def test_3_strong(self):
        ...
        for pc_order_id, s, order_status1, wei_order_id in zip(user_order_id, lever, order_status, pc_order_id_wei):
            ...
            if order_status1 == 4 or order_status1 == 3:
                ...
                开仓手续费
                '''
                sql_pc_match= "select * from pc_match where bid_order_id in ('%s');" % (wei_order_id)  # 根据订单id查询订单表，用户是否bid_order_id能查询的到
                sql_pc_match1 = "select * from pc_match where ask_order_id in ('%s');" % (wei_order_id)#用户是否用ask_order_id能查询到
                print(sql_pc_match)
                bid_uid = db_link.get_one(sql_pc_match)
                bid_uid1 = db_link.get_one(sql_pc_match1)
                if bid_uid1!=None:
                    #根据ask_order_id是不是等于-1，等于就用taker来算
                    if bid_uid1['is_taker']==-1:
                        ...
                        # 根据bid_order_id是不是等于1，等于就用maker来算
                ...
                #bid_order_id
                # 根据bid_order_id是不是等于-1，等于就用taker来算
                if bid_uid!=None:
                    # 根据bid_order_id是不是等于-1，等于就用taker来算
            ...
            '''
                ...
                if order_id['bs_flag'] == 2:
                 ...
                 计算余额
                  '''
                    ...
                    rise_Balance = sele_rise_user['futures_usdt_balance'] + order_id['amount']
                    # 多空仓强平价 = （多仓维持保证金+多仓合约数量*多仓开仓价格-余额）/（多仓合约数量）
                    duocang = (weichi + order_id['open_price'] * order_id['contract_qty'] - rise_Balance) / \
                              order_id['contract_qty']
                ...
                #判断用户是否是空仓
                    ...
                    sql_fund_user = "select * from fund_user t where uid in ('%s')" % (rise)
                    sele_fund_user = db_link.get_one(sql_fund_user)
                    # 余额 = 账户余额+空仓起始保证金
                    drop_Balance = sele_fund_user['futures_usdt_balance'] + order_id['amount']  # 用户余额

                    # 空仓强平价 = （空仓维持保证金-空仓合约数量*空仓开仓价格-余额）/（空仓合约数量）
                    kongcang = (weichi - order_id['contract_qty'] * order_id['open_price'] - drop_Balance) / \
                               order_id['contract_qty']
